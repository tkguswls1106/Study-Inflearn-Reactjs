...
npx create-react-app 프로젝트명(대문자 불가능)

- cd로 해당 프로젝트로 이동후 필요한 패키지 추가 설치 -
...
그리고 props를 활용하여 동적으로 스타일을 변경해주는것도 가능하다.
...
// div태그에 스타일넣은 props 사용한 Wrapper 컴포넌트 생성
const Wrapper = styled.div`
  padding: ${(props) => props.how};
`;
...
빌드란?: 작성한 코드와 애플리케이션이 사용하는 이미지, CSS파일 등의 파일을 모두 모아서 패키징 하는 과정이다.
...
npm run build
- 생성된 빌드파일로 웹앱 실행해보기 -
2. serve 설치하기 (serve는 생성된 빌드파일로 웹앱 실행전에 먼저, 빌드파일 안의 각각의 static 파일들을 서빙해주는 역할의 패키지이다.)
// 이건 -g라 글로벌모드로 설치라서, 각 프로젝트 폴더 내에 설치되는것이 아니라, 이 프로젝트뿐만이 아닌 이 컴퓨터의 다른 경로 어디에서든지 사용할수 있게 된다.
npm install -g serve  // 에러나면 sudo 붙여서 실행해보자
3. build 폴더를 기반으로 웹앱을 서빙한다. 이제 웹사이트 실행됨.
serve -s build

배포란?: 빌드를 통해 생성된 정적인 파일들을, 배포하려는 서버에 올리는 과정이다.
이렇게 배포하여 서버에 올려두고, serve 같은 명령어를 사용하여 서빙할 수 있게 해두면, 인터넷이 되는 어디에서든지 해당 서버 주소로 접속하여 웹사이트를 볼 수 있게 된다.
참고로 이 강의에서는 배포 과정은 다루지 않았다.
